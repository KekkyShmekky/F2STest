...
function GM:Think()
	...
	if self.WeaponSelectionTrigger and self.WeaponSelectionTrigger < CurTime() then
		if IsValid( self.SelectedWeapon ) then
			RunConsoleCommand( 'use', self.SelectedWeapon:GetClass() )
		end
...
function GM:hud_update_parameters()
	...
	if ply:InVehicle() then
		vec = ply:GetVehicle():GetAngles()
		vec:RotateAroundAxis( vec:Up(), 90 )
	end
...
function GM:CalcView( ply, origin, angles, ... )
	...
	if allow then view.angles.r = view.angles.r + math.Clamp( self.VelY * -0.03, -16, 16 ) end
